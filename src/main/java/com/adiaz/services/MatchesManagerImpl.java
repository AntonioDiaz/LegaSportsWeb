...
public class MatchesManagerImpl implements MatchesManager {
    ...
    @Override
    public List<String> initWeeksNames(long idCompetition) {
        List<String> weeksNames = new ArrayList<>();
        List<List<Match>> matchesByWeeks = queryMatchesOnWeeks(idCompetition);
        for (List<Match> matchesByWeek : matchesByWeeks) {
            Map<String, Integer> repetitions = new HashMap<>();
            for (Match match : matchesByWeek) {
                if (match.getDate() != null) {
                    String dateString = LocalSportsUtils.parseDateToString(match.getDate());
                    String dayStr = dateString.split(" ")[0];
    ...
    @Override
    public List<List<Match>> queryMatchesOnWeeks(Long idCompetition) {
        List<Match> matches = queryMatchesByCompetitionWorkingCopy(idCompetition);
        List<List<Match>> matchesByWeeks = new ArrayList<>();
        int prevWeek = -1;
        List<Match> matchTempList = null;
        for (Match match : matches) {
            int actualWeek = match.getWeek();
            if (prevWeek != actualWeek) {
                if (prevWeek!=-1) {
                    matchesByWeeks.add(matchTempList);
                }
                matchTempList = new ArrayList<>();
            }
            matchTempList.add(match);
            prevWeek = match.getWeek();
        }
        if (matchesByWeeks.size()>0) {
            matchesByWeeks.add(matchTempList);
        }
        return matchesByWeeks;
    }
}
